...
The guard: after the identifier's own authority answers, a candidate anchored on
the entry's OWN identifier, whose authors the entry confirms, whose title is a
different paper's, is distrusted unless a second identifier-anchored source
corroborates the divergence AND no identifier-anchored source confirms the entry's
title. Distrust removes the record from the candidate pool and is reported on
...
def _stub_candidate_pool(monkeypatch, checker: FactChecker, entry: dict, *pairs) -> None:
    """Feed ``check_entry`` an exact candidate pool while retaining its verdict logic."""
    pool = _candidates(checker, entry, *pairs)

    def fake_cascade(_entry, _query, sources_queried, sources_with_hits, _errors, _sources_failed):
        for _score, _record, source in pool:
    ...
    monkeypatch.setattr(checker, "_query_arxiv_by_id", lambda *_args, **_kwargs: [])
...
class TestCorruptionSignature:
    ...
    def test_corrupt_openalex_record_is_distrusted(self, logger, entry, work, true_arxiv):
        ...
        kept, distrusted = checker._split_corrupt_index_records(entry, candidates, authority_answered=True)
    ...
    def test_lone_corrupt_record_is_distrusted_without_a_counter_source(self, logger):
        ...
        kept, distrusted = checker._split_corrupt_index_records(LORA_ENTRY, candidates, authority_answered=True)
    ...
    def test_two_divergent_sources_corroborate_and_the_finding_stands(self, logger):
        ...
        checker = _checker(logger)
        corrupt = openalex_work_to_candidate_record(CORRUPT_LORA_WORK)
        semanticscholar_agreeing = PublishedRecord(
            doi="10.48550/arxiv.2106.09685",
            title="LoRA Fine-Tuning of a 3B Code LLM for Algorithmic Efficiency",
        ...
        candidates = _candidates(
            ...
            LORA_ENTRY,
            (corrupt, "openalex"),
            (semanticscholar_agreeing, "semanticscholar"),
        )
...
class TestArxivPrecedence:
    ...
    def test_openalex_title_does_not_override_arxiv_for_an_arxiv_doi(self, logger):
        ...
        kept, distrusted = checker._split_corrupt_index_records(TOOLLLM_ENTRY, candidates, authority_answered=True)
...
class TestVerdict:
    ...
    @pytest.mark.parametrize(
        "entry, work, true_arxiv, expected_status",
        [
            (TOOLLLM_ENTRY, CORRUPT_TOOLLLM_WORK, TRUE_TOOLLLM_ARXIV, FactCheckStatus.UNCONFIRMED),
            (CONSTITUTIONAL_ENTRY, CORRUPT_CONSTITUTIONAL_WORK, TRUE_CONSTITUTIONAL_ARXIV, FactCheckStatus.VERIFIED),
            (LORA_ENTRY, CORRUPT_LORA_WORK, TRUE_LORA_ARXIV, FactCheckStatus.UNCONFIRMED),
        ],
        ids=["toolllm", "constitutional-ai", "lora"],
    )
    def test_exact_status_and_the_distrust_is_reported(self, logger, entry, work, true_arxiv, expected_status):
    ...
    def test_arxiv_unreachable_keeps_the_only_candidate(self, logger):
        """Without an authority answer, the guard cannot blame the index."""
        checker = _checker(logger)
        checker.arxiv = None
    ...
    def test_noncrossref_doi_without_an_authority_answer_keeps_title_mismatch(self, logger, monkeypatch):
        entry = {**LORA_ENTRY, "doi": "10.5281/zenodo.9999999"}
        corrupt = openalex_work_to_candidate_record(CORRUPT_LORA_WORK)
        assert corrupt is not None
        corrupt = replace(corrupt, doi=entry["doi"], arxiv_id=None)
        checker = _checker(logger, config=FactCheckerConfig(check_dois=False))
        checker.arxiv = None
        _stub_candidate_pool(monkeypatch, checker, entry, (corrupt, "openalex"))
    ...
    def test_distrust_empty_pool_applies_strict_warn_cnv(self, logger, monkeypatch):
        config = FactCheckerConfig(strict=True, strict_warn_cnv=True)
        checker = _checker(logger, config=config)
        checker._arxiv_record_cache[TRUE_LORA_ARXIV.arxiv_id] = TRUE_LORA_ARXIV
        corrupt = openalex_work_to_candidate_record(CORRUPT_LORA_WORK)
        assert corrupt is not None
        _stub_candidate_pool(monkeypatch, checker, LORA_ENTRY, (corrupt, "openalex"))

        result = checker.check_entry(LORA_ENTRY)

        assert result.status is FactCheckStatus.STRICT_WARN_CNV
        assert len(result.distrusted_records) == 1

    def test_chimeric_detection_uses_candidates_distrusted_from_scoring(self, logger, monkeypatch):
        entry = {
            "ID": "chimera2017",
            "ENTRYTYPE": "article",
        ...
        config = FactCheckerConfig(check_dois=False, doi_consistency_min_title=0.40)
        checker = _checker(logger, config=config)
        monkeypatch.setattr(checker, "_structured_record_by_doi", lambda _doi: attention)
        _stub_candidate_pool(monkeypatch, checker, entry, (attention, "crossref"), (sae, "dblp"))

        result = checker.check_entry(entry)

        assert result.status is FactCheckStatus.HALLUCINATED
        assert result.chimeric_evidence is not None
        assert result.source_records == {"crossref": attention, "dblp": sae}
        assert len(result.distrusted_records) == 1

    def test_public_corroboration_author_and_fallback_branches(self, logger, monkeypatch):
        corrupt = openalex_work_to_candidate_record(CORRUPT_LORA_WORK)
        assert corrupt is not None

        checker = _checker(logger)
        checker._arxiv_record_cache[TRUE_LORA_ARXIV.arxiv_id] = TRUE_LORA_ARXIV
        _stub_candidate_pool(
            monkeypatch,
            checker,
            LORA_ENTRY,
            (corrupt, "openalex"),
            (corrupt, "semanticscholar"),
        )
        corroborated = checker.check_entry(LORA_ENTRY)
        assert corroborated.status is FactCheckStatus.TITLE_MISMATCH
        assert corroborated.distrusted_records == []
        ...
        _stub_candidate_pool(monkeypatch, checker, LORA_ENTRY, (other_paper, "openalex"))
        divergent_authors = checker.check_entry(LORA_ENTRY)
        assert divergent_authors.status is FactCheckStatus.NOT_FOUND
        assert divergent_authors.distrusted_records == []

        config = FactCheckerConfig(abstention_below=0.60)
        checker = _checker(logger, config=config)
        checker._arxiv_record_cache[TRUE_LORA_ARXIV.arxiv_id] = TRUE_LORA_ARXIV
        _stub_candidate_pool(
            monkeypatch,
            ...
            LORA_ENTRY,
            (corrupt, "openalex"),
            (corrupt, "openalex-fallback"),
        )

        fallback_duplicate = checker.check_entry(LORA_ENTRY)

        assert fallback_duplicate.status is FactCheckStatus.UNCONFIRMED
        assert len(fallback_duplicate.distrusted_records) == 2
...
def test_cli_can_disable_corrupt_index_distrust():
    args = build_parser().parse_args(["refs.bib", "--no-distrust-corrupt-index-records"])
    assert args.distrust_corrupt_index_records is False
