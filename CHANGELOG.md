...
- **A `@proceedings` volume could fail the consistency check on its own DOI.** `_check_doi_consistency` has a separate title comparison, so volume normalization applied elsewhere did not reach it. Publishers store the full descriptive title (`Computer Vision - ECCV 2016: 14th European Conference, Amsterdam, …, Proceedings, Part I`) against a cited `Computer Vision -- ECCV 2016`, scoring 0.36 and reading as a different work. Volume entries now normalize both sides as venue names here too, and the cited title being contained in the record's counts as confirmation; a genuinely wrong DOI on a volume still flags.
