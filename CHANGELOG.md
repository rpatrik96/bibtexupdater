...
An index that serves the right identifier and the right authors under the wrong title was read as evidence about the entry. This release distrusts that record instead, stops reporting a preprint retitled for its published version as a wrong arXiv ID, and says which sources a run actually queried rather than only which ones hit.

Additive for consumers: `api_sources_queried` and `distrusted_records` join both report shapes, and no existing key changes.

### Fixed

- **The library-only async OpenReview stage was anonymous, single-host, and silent about it.** `AsyncResolver._openreview_search` -- stage 3c of the preprint resolver -- ran on `AsyncHttpClient`, which had no `openreview_auth`, so every request went out without a token, took the `403` challenge, and the method returned `[]` without a word. It also queried `api.openreview.net` alone, so even an authenticated fix would have missed every 2023+ venue. The 1.9.0 through 1.10.2 fixes (#62, #63, #65, #68) reached `bibtex-check` only; this is the same class of defect in the one path they did not touch (#67). `AsyncHttpClient` now takes the same `OpenReviewAuth` object `HttpClient` does -- built from `OPENREVIEW_USERNAME` / `OPENREVIEW_PASSWORD` or `--openreview-username`, exactly as before -- and treats it the same way: a bearer token on every `service="openreview"` request, one re-login on a `401`, no login spent on a `403`. A library caller constructing `AsyncHttpClient` must pass `openreview_auth=OpenReviewAuth.from_env(...)` itself. The login POST is synchronous and takes the per-origin lock, so it runs through `asyncio.to_thread` rather than on the event loop; reusing the sync `token_for_url` (instead of an async re-implementation) means the updater shares the one cached token, in memory and in the cross-process store, with the checker. The async stage now asks both hosts (v2 first, then v1, as the sync client does), issues both index forms of the `paperhash` from the raw first-author name, and paces the `/notes/search` fallback under its own 5/min limiter. Refusals follow the 1.10.1 rule: an anonymous `401`/`403` latches that endpoint for the run and is reported once, at WARNING, with the two variables that fix it; an authenticated one is reported once and forgotten, so the next entry asks the same host again. Without credentials the behaviour is unchanged apart from that log line and the second host: the anonymous-capable full-text fallback still runs, so OpenReview keeps contributing what it can. The sync `bibtex-update` stage 3c also now passes the raw first-author name, so an accented author is hashed under both index forms there too.

- **`preprint_only` used the clearly-correct confidence anchor despite its problem polarity.** Its confidence moved from 0.88 to the 0.78 soft-problem tier, so `p_valid` moved from 0.060 to 0.110. `doi_not_found` and `url_accessible` retain their values but now use weak anchors with the correct polarity.

- **Venue comparison now expands common ISO-4 journal abbreviations and abstains on unrecognized venue pairs.** The terminal branch returns `NON_COMPARABLE` instead of `MISMATCH`, moving those fields from `mismatched_fields` to `unconfirmed_fields`. On a workshop corpus, 920 of 5,043 references had venue as the sole disagreement, including 127 with no journal field and a correct non-canonical venue. Real wrong venues accounted for 8 of 119 field-level defects, so known-different venues still return `MISMATCH`.
...
- Invalid `BIBTEX_ARXIV_RATE` values no longer abort the checker before lookups begin. The CLI logs the rejected value and uses the default rate of 20 requests per minute.

- **`bibtex-update`'s OpenReview stage was anonymous, single-host, and silent about it.** `Resolver._openreview_search` -- stage 3c of the preprint resolver -- ran on `AsyncHttpClient`, which had no `openreview_auth`, so every request went out without a token, took the `403` challenge, and the method returned `[]` without a word. It also queried `api.openreview.net` alone, so even an authenticated fix would have missed every 2023+ venue. The 1.9.0 through 1.10.2 fixes (#62, #63, #65, #68) reached `bibtex-check` only; this is the same class of defect in the one path they did not touch (#67). `AsyncHttpClient` now takes the same `OpenReviewAuth` object `HttpClient` does -- built from `OPENREVIEW_USERNAME` / `OPENREVIEW_PASSWORD` or `--openreview-username`, exactly as before -- and treats it the same way: a bearer token on every `service="openreview"` request, one re-login on a `401`, no login spent on a `403`. The login POST is synchronous and takes the per-origin lock, so it runs through `asyncio.to_thread` rather than on the event loop; reusing the sync `token_for_url` (instead of an async re-implementation) means the updater shares the one cached token, in memory and in the cross-process store, with the checker. The async stage now asks both hosts (v2 first, then v1, as the sync client does), issues both index forms of the `paperhash` from the raw first-author name, and paces the `/notes/search` fallback under its own 5/min limiter. Refusals follow the 1.10.1 rule: an anonymous `401`/`403` latches that endpoint for the run and is reported once, at WARNING, with the two variables that fix it; an authenticated one is reported once and forgotten, so the next entry asks the same host again. Without credentials the behaviour is unchanged apart from that log line and the second host: the anonymous-capable full-text fallback still runs, so OpenReview keeps contributing what it can. The sync `bibtex-update` stage 3c also now passes the raw first-author name, so an accented author is hashed under both index forms there too.
...
- **A chimeric-title `HALLUCINATED` verdict now carries the evidence behind it, and its confidence is measured rather than fixed** (#70). `_detect_chimeric_title` returned a bare bool and `check_entry` turned it into the tool's strongest verdict at a hard-coded 0.95 with `field_comparisons`, `best_match` and `source_records` all empty, so nothing downstream could re-examine which two sources disagreed, what titles they returned, or which tokens drove the call. The detector now returns a `ChimericEvidence` record (the two source names, their matched titles and DOIs, per-source scores, and the shared and unique token sets, sorted) or `None`; the detection rule and its thresholds -- each source shares >= 4 non-stopword tokens with the entry title and contributes >= 3 the other does not -- are unchanged, and when several source pairs qualify the pair with the largest margin is reported. On the result, `best_match` is the higher-scoring of the two records, `source_records` holds both keyed by source, `field_comparisons["title"]` is a `MISMATCH` whose note spells out the token sets (so `mismatched_fields` now lists `title` for these entries, where it was empty), the error string names both sources and their token counts, and the additive `chimeric_evidence` field is emitted under that key in the JSON and JSONL reports only when present -- every pre-existing key is untouched. `overall_confidence` is now `chimeric_confidence(min_shared, min_unique)`: with `margin = (min_shared - 4) + (min_unique - 3)`, confidence is `0.80 + 0.17 * margin / (margin + 4)`, so a threshold-exact 4/3 detection reports 0.80, a 9/7 detection 0.918, and the value approaches but never reaches 0.97. `p_valid` follows it through the existing problem-polarity mapping.

### Documentation

- **`--strict` is stricter than the NeurIPS 2026 hallucinated-reference criteria, and the documentation now says where the two lines differ.** `docs/REFERENCE_FACT_CHECKER.md` records which problematic statuses those criteria place in their hallucinated class (`hallucinated`, `nonexistent_venue`, `title_mismatch`, and the fabrication side of `author_mismatch`) and which they treat as errors to report to the authors, citing the NeurIPS 2026 program chairs' message to authors of 5 September 2026. The README's hallucination-detection bullet carries the same caveat and links to it.
...
- **An arXiv identifier beside a real published venue hid that venue.** `entry_venue` read `journal` before `booktitle`, so an entry carrying both `journal = {arXiv preprint arXiv:1706.03762}` (for findability) and `booktitle = {Advances in Neural Information Processing Systems}` (the actual claim) was scored against the preprint string and the published claim never constrained anything. A preprint-server string now loses to a real venue wherever the entry carries both; field precedence is otherwise unchanged, and an entry whose only venue field is a preprint string still returns it. An identifier in `eprint`/`archiveprefix`/`note`/`url` or a `10.48550/arXiv.*` DOI never reached the venue comparison and never could.
- **Brace-protected venue names hard-mismatched.** Zotero/Better BibTeX writes `The {{Twelfth International Conference}} on {{Learning Representations}}`. `_normalize_venue_for_matching` never ran a LaTeX pass, so the braces stayed glued to the tokens, no alias matched, and the fuzzy score against `ICLR` fell to 0.09 -- a `VENUE_MISMATCH` verdict on a correctly cited ICLR paper. It now runs `latex_to_plain` first, which also decodes accents and unwraps commands. The alias map itself was never the problem: `International Conference on Learning Representations`, the same name with an `(ICLR)` gloss, and the `Advances in Neural Information Processing Systems (NeurIPS)` form all canonicalized correctly before this change.
- **COLM was missing from the venue alias map**, so `Conference on Language Modeling` -- with or without a `(COLM)` gloss, with or without an ordinal edition prefix -- canonicalized to nothing and could reach `VENUE_MISMATCH` against a correctly cited paper. Added with the ordinal prefixes handled by the existing substring pass.

### Changed

- **`mismatched_fields` in the JSON and JSONL reports now lists real contradictions only.** It was built from the legacy two-valued `matches` flag, which is false for `NON_COMPARABLE` and `PARTIAL` as well as for `MISMATCH`, so a field the checker had deliberately declined to compare was reported under a key named for findings against the entry. 956 of the 1,024 venue-only entries in the corpus above were abstentions of that kind, and downstream they read as a venue the checker had refuted. The abstentions moved to a new additive `unconfirmed_fields` key on both report shapes; no other key changes, and the statuses were always correct (`UNCONFIRMED`, not `VENUE_MISMATCH`).
...
- **The two OpenReview hosts each bought their own login, and OpenReview refuses the fourth in two minutes.** One JWT authenticates both: verified live, a token minted at `api2.openreview.net/login` answers 200 on `api.openreview.net/notes?paperhash=` and on `api2/notes`, where an anonymous caller is refused on both. The client now presents a token held for either host at both, in memory and from the cross-process cache alike, and logs in only when neither host has one. `invalidate` drops the shared string everywhere it is held, so an expired token cannot survive at the sibling host and be handed to the next lookup. A run that queries both hosts per entry therefore spends one login instead of two, which is what keeps a sharded fleet clear of the `429`.
...
- **Logins were spent per process, and OpenReview refuses the fourth in two minutes.** The client logged in once per process and never reused the token, which is right for one long run and wrong for a sharded one: three shards over one bibliography produced repeated logins, and OpenReview refused enough of them with `429` to degrade those runs to anonymous. The token is valid for 24 hours and authenticates both hosts, so it is now cached across processes and the whole fleet logs in once. A `401 TokenExpiredError` refreshes it exactly once per request and drops the shared copy; a `403 ChallengeRequiredError` means the request was never recognized as authenticated, so it no longer spends a login answering a question that was not asked.

### Added

- **`BIBTEX_CHECK_OPENREVIEW_TOKEN_CACHE`** controls the cross-process token cache: unset uses the default path, a path moves the file, and `0`/`off`/`false`/`none` turns persistence off. The file is written atomically at mode 0600, in a cache directory this run creates at 0700 when it does not already exist. It holds no password and no username. Every failure is non-fatal: an unreadable or unwritable cache degrades to the login-per-process behaviour it replaced.
...
- **A network outage was reported as thousands of references no database has heard of.** Every source client caught its own failures and returned an empty result, so a DNS failure, refused connection, TLS error, read timeout or exhausted 429/5xx retry budget was indistinguishable downstream from a database that had checked and found nothing. Mid-run wifi loss during a 5,043-reference check therefore produced `not_found` for 2,500 real, correctly-cited references, at exit code 0 with nothing logged. In citation screening `not_found` is the status that feeds "this reference may be fabricated", so the error pointed the worst possible way. The clients now raise `SourceUnavailableError` for a lookup that ends without an answer, the cascade records which sources failed per entry, and `not_found` is demoted to `api_error` whenever any of them did, including when the remaining sources answered cleanly and found nothing, because a partial cascade cannot establish an exhaustive miss. A source that answers with zero results is unaffected and still yields `not_found`. Failed lookups are logged with the affected sources, and the unreachable hosts named separately: an HTTP error response proves the network is up, so a 429 is a refusal to answer rather than an outage.

- **A host that never answered, or refused to, read as a dead web page.** `url_not_found` — whose own definition read "404 or domain unreachable" — was returned for a DNS failure, refused connection, TLS error or timeout, the same conflation as above in the web-reference family. It was also returned for every other non-2xx, so a bot-blocking 403 on an academic URL and a proxy's 503 both read as a dead citation. Only a status that asserts absence now earns the verdict: HTTP 404 and 410 stay `url_not_found`, while an unreachable host, a 401/403 refusal, a 429 and a 5xx report `api_error` with `sources_failed: ["url_check"]`. A host that is up and talking has said nothing about the page. `url_check.lookup_failed` in the JSON report records which of the two happened.

### Added

- **`--outage-threshold FLOAT`** sets the fraction of entries (0–1) with a failed source lookup at which a run exits 5, defaulting to the previous fixed 10%. `0` fails on a single failed lookup; `1` fails only when every entry was affected. There is no value that switches the gate off: the threshold decides when a degraded run becomes a condemned one, never whether failed lookups are reported.
...
- **A volume cited by its short title read as a different work.** Indexes store a proceedings volume under its full descriptive title — the meeting's place and dates (`…Computational Linguistics, ACL 2020, Online, July 5-10, 2020`) or a part number (`Computer Vision - ECCV 2016: 14th European Conference, Amsterdam, …, Proceedings, Part I`) — while bibliographies cite the short form. Scores of 0.86 and 0.36 followed, on volumes carrying the correct DOI. For a `@proceedings` entry the cited title being contained in the indexed one now counts as confirmation, in both the field comparison and the DOI-consistency check (which has its own title comparison and would otherwise be missed). Containment is sound only because the entry side is a volume title; for a paper title a record that merely contains it would be a different, longer-titled work.
