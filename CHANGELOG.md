...
### Tests
- +18 tests in `tests/test_subtle_failures.py` (particle-surname symmetry, impossible-month arXiv-ID rejection, legacy/`.pdf` URL extraction, single-token venue exact-matching); plus arXiv-ID↔title consistency, resolver `_verify_arxiv_match`, and DBLP-suffix/`CoRR`-venue regression tests. Full suite: 781 passed, 1 skipped (zero regressions).
