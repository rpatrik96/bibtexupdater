#!/usr/bin/env python3
"""
filter_bibliography_minimal.py - Standalone bibliography filtering for LaTeX projects.

A minimal-dependency alternative to filter_bibliography.py that uses only
Python standard library. Designed for use on Overleaf and other restricted
environments where pip install is not available.

Usage:
    python filter_bibliography_minimal.py paper.tex -b references.bib -o filtered.bib
    python filter_bibliography_minimal.py *.tex -b references.bib -o filtered.bib
    python filter_bibliography_minimal.py ./chapters/ -b references.bib -o filtered.bib -r

    # Multiple bib files (merged, errors on duplicates):
    python filter_bibliography_minimal.py paper.tex -b refs1.bib refs2.bib -o filtered.bib

For latexmkrc END block integration (works on Overleaf with pdfLaTeX):
    END {
      system("python3 filter_bibliography_minimal.py . -b refs.bib -o refs_filtered.bib -r --no-warn-missing");
    }
