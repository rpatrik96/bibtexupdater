"""Claude API classifier backend using raw HTTP requests."""
...
class ClaudeClassifier(AbstractClassifier):
    """Classifier using Claude API for paper topic classification."""
    ...
    def __init__(self, config: ClassifierConfig) -> None:
        ...
        if not self.api_key:
            raise ValueError("Claude API key required. Set ANTHROPIC_API_KEY or pass api_key in config.")
    ...
    def _call_api(self, prompt: str) -> dict[str, Any]:
        ...
        response = self.client.post(CLAUDE_API_URL, headers=headers, json=payload)
    ...
    def estimate_cost(self, num_papers: int, avg_abstract_length: int = 500) -> float:
        ...
        pricing = CLAUDE_PRICING.get(self.model, CLAUDE_PRICING["claude-sonnet-4-20250514"])
